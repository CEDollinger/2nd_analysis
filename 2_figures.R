  ...
  for (i in 1:3) {
    ...
    p3a <- p2 %>% 
      add_trace(data = baseline.drivers, x= ~dist, y= ~regen, # baseline driver rates by landscape
                mode = "markers", type = "scatter", name = "Baseline\nposition", 
                marker = list(size = 30, color = "black", symbol = "x")) %>%
      layout(legend = list(font = list(size = 40))); p3a
    p3b <- p2 %>%
    ...
    if (i == 3 | clim == "hotdry") {
      save_image(p3b, file = paste0("results/figures/suppl_figures/Q2_contourPlot_loess_", i, "_", clim, ".png"), scale=1, 
                 width=1900, height=1700) 
    } else {
      save_image(p2, file = paste0("results/figures/Q2_contourPlot_loess_", i, "_", clim, ".png"), scale=1, 
                 width=1900, height=1700) 
    } 
    rm(p1, p2, p3a, p3b, data.loess, a, mtrx.melt, xgrid, ygrid, data.fit, mtrx3d, hulls, centroids, hulls_geom, hulls_sf, centroid_coords) #, p3
  }
}
...
mtrx.ls <- list(list(NA, NA, NA), list(NA, NA, NA)); names(mtrx.ls) <- c("baseline", "hotdry")
hull.ls <- list(NA, NA); names(hull.ls) <- c("baseline", "hotdry")
i<-1; clim <- "baseline"
for (clim in c("baseline", "hotdry")) {
  for (i in 1:3) {
    a <- dyn.effect %>% 
      filter(name == names(response.colors)[i]) %>% 
      dplyr::select(paste0("dist.dyn_", clim), paste0("regen.dyn_", clim), effect, landscape) %>% 
      rename(dist.dyn = 1,
             regen.dyn = 2) %>% 
    ...
    mtrx.melt$regen.dyn <-  as.numeric(str_sub(mtrx.melt$regen.dyn, str_locate(mtrx.melt$regen.dyn, '=')[1,1] + 1))
    head(mtrx.melt)
    mtrx.ls[[clim]][[i]] <- mtrx.melt
    rm(mtrx.melt)
  }
}

scale.ls <- list(NA, NA); names(scale.ls) <- c("baseline", "hotdry")
for (clim in c("baseline", "hotdry")) {
  ...
  set.seed(42)
  colorlength <- 100
  null_value <- (0 - min(bind_rows(mtrx.ls[[clim]])$effect)) / (max(bind_rows(mtrx.ls[[clim]])$effect) - min(bind_rows(mtrx.ls[[clim]])$effect))
  ...
  scale.ls[[clim]] <- colorscale
}; rm(colorscale)

for (clim in c("baseline", "hotdry")) {
  for (i in 1:3) {
    p1 <- plot_ly(mtrx.ls[[clim]][[i]], x = ~dist.dyn, y = ~regen.dyn, z = ~effect, type = "contour",
                  colorscale = scale.ls[[clim]], contours = list(showlines = FALSE)
      ...
      layout(#title = paste0(names(response.colors)[i]), 
        ...
        yaxis = list(title = 'Recruitment rate [recruited ha^-1 yr^-1]',
                     ...
                     mirror = T,
                     tickvals = c(1:5*10),
                     range=range(mtrx.ls[[clim]][[i]]$regen.dyn, na.rm = TRUE),
                     titlefont = list(size = 50), tickfont = list(size = 50))) %>% 
      colorbar(title = "Climate effect [%]", limits = c(min(bind_rows(mtrx.ls[[clim]])$effect), max(bind_rows(mtrx.ls[[clim]])$effect)),
               titlefont = list(size = 50), tickfont = list(size = 50)); p1
    p2 <- p1 %>%
      add_trace(data = hull.ls[[clim]], x = ~dist.dyn, y = ~regen.dyn, 
                mode = "lines", type = "scatter",
                line = list(width = 2),  # Line width
                color = ~factor(landscape),
                colors="grey30",
    ...
    if (i == 3 | clim == "hotdry") {
      save_image(p2, file = paste0("results/figures/suppl_figures/Q3_contourPlot_loess_", i, "_climateEffect_", clim, "Hulls.png"), scale=1, 
                 width=1900, height=1700)
    } else {
      save_image(p2, file = paste0("results/figures/Q3_contourPlot_loess_", i, "_climateEffect_", clim, "Hulls.png"), scale=1, 
                 width=1900, height=1700)
      
    }
  }
  
}; rm(p1, p2, data.loess, a, xgrid, ygrid, data.fit, mtrx3d, border, border_neg, border_pos, s, i, mtrx.ls, hull.ls, scale.ls,
