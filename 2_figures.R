...
singleProcess.df <- read_csv("results/datasets/singleProcess.df.csv") %>%
  ...
  mutate(
    process = case_match(
      process,
      "Seed production decrease" ~ "Seed production",
      "Sapling growth limitations" ~ "Sapling growth",
...
for (i in 1:3) {
  a <- dyn.df %>%
    ...
    mutate(
      value = 100 - value * 100, # convert from proportion of unchanged landscape to % of changed landscape
  ...
  p1 <- plot_ly(mtrx.melt,
    ...
    layout(
      ...
      yaxis = list(
        title = "Regeneration rate [recruited ha^-1 yr^-1]",
        tickvals = c(1:5 * 10),
...
for (i in 1:3) {
  ...
  # Generate a dataframe with every possible combination of wt and hp
  data.fit <- expand.grid(dist.dyn = xgrid, regen.dyn = ygrid)
  # Feed the dataframe into the loess model and receive a matrix output with estimates of climate effect for each combination of disturbance and regeneration rate
  mtrx3d <- predict(data.loess, newdata = data.fit)
  # Abbreviated display of final matrix
  mtrx3d[1:4, 1:4]
...
for (i in 1:3) {
  p1 <- plot_ly(mtrx.ls[[i]],
    ...
    colorbar(
      title = "Climate effect [%]", limits = c(min(bind_rows(mtrx.ls)$effect), max(bind_rows(mtrx.ls)$effect)),
      titlefont = list(size = 50), tickfont = list(size = 50)
    )
  p1
...
rm(
  p1, p2, data.loess, a, xgrid, ygrid, data.fit, mtrx3d, border, border_neg, border_pos, s, i, mtrx.ls, hulls, bgd.hull, grte.hull, colorscale,
